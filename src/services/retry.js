// Cadena de modelos Gemini con fallback y reintentos para errores transitorios.
// La cuota es por modelo, así que cuando el principal se agota (429) o está
// saturado (503) se pasa al siguiente. Con 429 el salto es inmediato, porque
// reintentar el mismo modelo no va a ayudar; con 503 se reintenta antes con
// backoff. La cadena se configura con GEMINI_MODELS, separada por comas.
  ...
  for (let m = 0; m < MODELS.length; m++) {
    ...
    for (let attempt = 0; attempt <= MAX_RETRIES_PER_MODEL; attempt++) {
      try {
        ...
        if (!RETRYABLE.includes(err.status) && !timeout) throw err // no es transitorio, fallar ya
