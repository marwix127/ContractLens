// Cliente de la API de ContractLens. En desarrollo, Vite hace proxy de
// /contracts al backend Express (ver vite.config.js).

export async function uploadContract(file) {
  const form = new FormData()
  form.append('file', file)
  const res = await fetch('/contracts', { method: 'POST', body: form })
...
export async function analyzeContract(contractId) {
  const res = await fetch(`/contracts/${contractId}/analyze`, { method: 'POST' })
  const data = await res.json().catch(() => ({}))
...
export async function streamChat(contractId, question, conversationId, handlers = {}) {
  const res = await fetch(`/contracts/${contractId}/chat/stream`, {
    method: 'POST',
